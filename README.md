...
**[설치 파일 다운로드](https://github.com/nicepong1/codex-session-bridge/releases/tag/v0.19.8)** · **[사용설명서](docs/USER-GUIDE.md)** · **[문제 해결](docs/TROUBLESHOOTING.md)** · **[앱 개요와 구조](docs/OVERVIEW.md)**

## 할 수 있는 일

- 호스트의 기존 작업과 프로젝트 목록 조회, 작업 선택, 본문 사전 읽기와 긴 대화의 과거 내용 불러오기
- GPU 공식 앱에서 직접 갱신했지만 최근 목록 색인에서 빠진 사용자 작업 복원
- 같은 작업에 텍스트·이미지 지시 전송, 새 작업 생성 및 첫 입력 전달
- 모델과 추론 수준 변경, 호스트 프로젝트 등록
- 명령·파일 변경·추가 권한 승인, 질문 답변, 일반 MCP 폼·URL 확인 전달
- 원본 계획 실행과 미지원 요청 안내. 상세 조건은 승인 지원표에 표시
...
| 호스트 Codex 앱 | 26.901.5280.0, 26.901.6511.0, 26.903.8094.0, 26.908.4834.0, 26.915.4065.0 |
| 호스트 공식 CLI | codex-cli 0.153.4, 0.154.0-alpha.6.2, 0.155.0-alpha.9.2 |
| Android | 공식 ChatGPT Remote를 별도로 사용. 이 ZIP의 설치 대상 아님 |
| macOS/Linux/Windows ARM64 | 미지원 |
| 여러 호스트 | 설정 저장 및 전환 지원. 한 사용자에게 연결 창 하나만 실행 |

**옵션 선택기·일부 초기 설정, 브라우저·계정 인증, 플러그인 설치 제안, 파일 선택 폼, 계획 편집본 실행·닫기는 호스트 공식 앱에서 처리해야 합니다.** 대기 메시지 추가·실행, 실행 중 중지·개입, 일반 파일 첨부·원격 파일/터미널 패널도 미지원입니다. 텍스트와 함께 보낸 이미지 첨부는 최대 4개·합계 8MB까지 지원합니다. 자세한 범위와 실제 화면 검증 여부는 [승인·질문 지원표](docs/APPROVALS.md)를 참고하세요.
