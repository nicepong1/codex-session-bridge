...
export function firstTaskTurn(raw) {
  ...
  if (!Array.isArray(raw.input) || raw.input.length!==1 || raw.input[0]?.type!=='text' ||
      typeof raw.input[0].text!=='string' || !raw.input[0].text.trim() || raw.input[0].text.length>16000 || raw.input[0].text_elements?.length)
    fail('First input must be one text message, up to 16000 characters');
  ...
  return {threadId:raw.threadId,operationId:raw.clientUserMessageId,text:raw.input[0].text,settings:settingsFromTurn(raw)};
}
