...
[IO.File]::WriteAllText($bridgeManifest,(@{version='0.19.6';files=$bridgeEntries}|ConvertTo-Json -Depth 4))
...
try {
 ...
 $bridgeTarget=Join-Path $env:LOCALAPPDATA ('Programs\CodexSessionBridge\versions\0.19.6-'+$bridgeHash)
 ...
 foreach($bridgeCase in @('legacy','unknown','changed','active','pointed','linked')){
  ...
  $script:bridgeTarget=Join-Path $env:LOCALAPPDATA ('Programs\CodexSessionBridge\versions\0.19.6-'+$bridgeHash)
